...
| `train-cycle` | Loop download → train → holdout → policy → run (`--minutes`, `--max-cycles`) |
...
- `epoch_ai/execution/policy/` — baseline ensemble + PPO + guardrails (RL boundary; see ADR 0008). **Shared-trunk (A.5):** set `rl.observation_mode: embedding` to train/run PPO on TCN trunk embeddings (`trunk_policy.py`, `TCNModel.embed()`, `runtime_trunk_embedding`). Stage 1 keeps `trunk_frozen: true` (default); Stage 2 joint fine-tune sets `trunk_frozen: false` and `policy_loss_weight > 0` so alternating PPO + supervised aux steps run via `learning/trunk_joint_train.py`. Promotion vetoes joint challengers when holdout Brier/AUC regress beyond `rl.promotion.max_prediction_brier_regression` / `max_prediction_auc_regression`; registers the fine-tuned TCN when promotion passes. Policy load validates `obs_dim` vs config. See ADR 0009.
